# EPUB2md — convert EPUB books into Reboku Markdown books (.md)
#
...
"""EPUB -> Reboku Markdown book (.md)

EPUB2txt's sibling (owner 2026-09-02): the SAME analysis layer — chapters from the book's own
table of contents, footnotes paired by id, sub-headings from the stylesheet, the cover, the
encoding — with the output written as CommonMark plus Reboku's harmless extensions
(see README.md, 「輸出格式」). Every file it writes is valid CommonMark: another tool
opens it and merely loses the extensions.

    python EPUB2md.py                    # no source -> opens the window (GUI)
    python EPUB2md.py book.epub
    python EPUB2md.py ./library -r -o ./out -f

輸出檔名與來源同名(book.epub -> book.md);`-o` 加上資料夾來源時,來源的子資料夾結構會照樣建在
輸出資料夾底下(`--flat` 則全部攤平)。已存在的檔案會先問;`-f` 直接覆寫。
The output is named after the source file (book.epub -> book.md); with `-o` on a folder the
sub-folders are mirrored (`--flat` puts everything in one folder). Existing files are asked
about; `-f` overwrites.

samples/(Sample.epub 轉出來就是 Sample.md):
    python EPUB2md.py samples/Sample.epub -o /tmp/out
converting Sample.epub reproduces Sample.md byte for byte.
...
The format the reader accepts is described in README.md (「輸出格式」).
"""
...
COVER_MAX = (200, 300)  # DEFAULT box the picture is fitted INSIDE; the ratio is kept
COVER_QUALITY = 80  # DEFAULT JPEG quality
# Both are defaults only (owner 2026-09-23): `--cover-size WxH` / `--cover-quality N` on the
# command line, and the two fields beside the cover tick-box in the window, override them per run.
COVER_QUALITY_RANGE = (1, 95)  # Pillow's usable JPEG range; above 95 the file only grows


def parse_cover_size(text: str) -> tuple[int, int]:
    """`"200x300"` -> (200, 300). Two positive integers joined by `x`, `X` or `*`."""
    parts = re.split(r"[xX*]", str(text).strip())
    if len(parts) != 2 or not all(p.strip().isdigit() for p in parts):
        raise ValueError(f"cover size must look like 200x300, not {text!r}")
    width, height = (int(p) for p in parts)
    if width <= 0 or height <= 0:
        raise ValueError(f"cover size must be positive, not {text!r}")
    return width, height


def parse_cover_quality(text: str) -> int:
    """`"80"` -> 80, kept inside Pillow's usable JPEG range."""
    try:
        value = int(str(text).strip())
    except ValueError:
        raise ValueError(f"cover quality must be a whole number, not {text!r}") from None
    low, high = COVER_QUALITY_RANGE
    if not low <= value <= high:
        raise ValueError(f"cover quality must be between {low} and {high}, not {value}")
    return value
...
def cover_jpeg_base64(book: Epub, size: tuple[int, int] = COVER_MAX, quality: int = COVER_QUALITY) -> str:
    """The book's cover as a JPEG fitted inside `size` (default 200x300), base64'd. "" when there is none.
    ...
    try:
        ...
        picture.thumbnail(size)  # fits the box, keeps the aspect ratio, never enlarges
        out = io.BytesIO()
...
def _yaml_value(value: str) -> str:
    """One front-matter value. The reader's parser takes the rest of the line after `key:` and
    strips ONE pair of surrounding quotes, so quoting is only needed when the raw text would be
    misread: empty, or wrapped in quote characters of its own."""
    text = value.replace("\r", " ").replace("\n", " ").strip()
    if not text or text[0] in "\"'" or text[-1] in "\"'":
        # Bare wrapping, NO escaping inside the reader's `unquote` only
        # strips one outer pair and knows no escape syntax, so `"` → `"""` → `"` round-trips while
        # a `\"` would surface in the library as a literal backslash.
        return '"' + text + '"'
    return text


def _front_matter(book: Epub, fallback_title: str, guessed_date: str, cover_b64: str) -> str:
    """The `---` header: book information as `key: value` lines (README.md 「表頭」).

    Only the two line shapes the reader reads — `key: value` and `- item` — so no YAML library
    is needed on either side. The cover travels as ONE `data:` line: a JPEG, because that is what
    [cover_jpeg_base64] makes (the reader also accepts PNG in hand-written files).
    """
    lines = ["---", f"title: {_yaml_value(book.title or fallback_title)}"]
    if len(book.authors) == 1:
        lines.append(f"author: {_yaml_value(book.authors[0])}")
    elif book.authors:
        lines.append("author:")
        lines.extend(f"- {_yaml_value(author)}" for author in book.authors)
    if book.publisher:
        lines.append(f"publisher: {_yaml_value(book.publisher)}")
    date = book.date or guessed_date
    if date:
        lines.append(f"date: {_yaml_value(date)}")
    if book.language:
        lines.append(f"language: {_yaml_value(book.language)}")
    if book.series:
        lines.append(f"series: {_yaml_value(book.series)}")
    if book.series_index:
        lines.append(f"series_index: {_yaml_value(book.series_index)}")
    if cover_b64:
        lines.append(f"cover: data:image/jpeg;base64,{cover_b64}")
    lines.append("---")
    return "\n".join(lines)
...
# Two ideographic spaces open EVERY paragraph, in every language. To
# CommonMark an ideographic space is ordinary text, so the indent survives as an indent; four
# ASCII spaces would start a code block, which is why the Latin-script indent of the .txt format
# cannot be carried over. The reader adds no text-indent of its own on this route.
MD_INDENT = "　　"

# ASCII punctuation CommonMark gives a meaning inside prose (CommonMark §2.4: a backslash before
# ASCII punctuation makes it literal). A book's own `*`, `_`, `[1]`, `<b>`, `&amp;` or `~~` would
# otherwise turn into emphasis, links, raw HTML, entities or strikethrough. `#`, `-`, `+`, `>`
# and `1.` matter only at the START of a line, and every body line starts with the indent, so
# they need nothing. `|` is escaped too: a stray pipe is harmless, a row of them is a table.
_MD_SPECIAL = re.compile(r"([\\`*_\[\]<>~&|])")

# What this program itself writes into a line and must NOT escape: a fenced address (a CommonMark
# autolink — `<scheme:…>` with a 2–32 character scheme, CommonMark §6.4, so `<mailto:…>` counts as
# much as `<https://…>`) and a note marker (turned into `[^label]` by [_finish_markdown]).
#
# ⚠️ Only the schemes the analysis layer actually writes (`<http…>`, `<https…>`, `<mailto:…>`,
# `<ftp…>`). "Any scheme-looking thing" turned a book's own prose
# `<dc:title>` into a link and swallowed its angle brackets — that text is the author's and must
# be escaped like any other `<…>`.
_MD_PROTECTED = re.compile(
    r"(<(?i:https?|mailto|ftps?):[^<>\s]*>|\((?:註|注|주|Note)\[[^\]\n]{1,16}\]\))")


def md_escape(text: str) -> str:
    """Body text made safe for CommonMark, with the program's own tokens left intact."""
    parts = _MD_PROTECTED.split(text)
    return "".join(part if i % 2 else _MD_SPECIAL.sub(r"\\\1", part) for i, part in enumerate(parts))


def paragraph_indent(language: str) -> str:  # noqa: ARG001 - kept for the call sites
    """The first-line indent — the same for every language in the Markdown format."""
    return MD_INDENT


def convert(path: Path, with_cover: bool = True, cover_size: tuple[int, int] = COVER_MAX,
            cover_quality: int = COVER_QUALITY) -> tuple[str, str]:
    ...
    cover = cover_jpeg_base64(book, cover_size, cover_quality) if with_cover else ""
    front = _front_matter(book, path.stem, guessed_date, cover)
    # No Contents block: a Markdown reader builds the table of contents from the headings
    # `emitted` is still tracked so a corpus comparison can count chapters.
    text = _join_blocks(_finish_markdown(body))
    return book.title or path.stem, front + "\n\n" + text + "\n"
...
BLANK_BLOCK = ("blank",)  # a whitespace-only paragraph from the source: one blank line
RULE_BLOCK = ("rule",)  # a horizontal rule the book drew

# Sub-headings must never become chapters: the reader cuts chapters at `#` and `##` only, so an
# in-chapter heading is `###` at the shallowest, one level under its chapter when deeper.
SUBHEAD_MIN_LEVEL = 3

# A note line the analysis layer wrote (`註[2]: text`, in the book's own word) and a marker it
# put into a sentence (`(註[2])`). Both are recognised here EXACTLY as the .txt reader would.
NOTE_LINE_RE = re.compile(r"^(?:註|注|주|Note)\[([^\]\n]{1,16})\]:[ 　]?(.*)$")
NOTE_MARK_RE = re.compile(r"[ 　]*\((?:註|注|주|Note)\[([^\]\n]{1,16})\]\)")
# Footnote labels may not hold spaces or `]`; anything else is fine for markdown-it-footnote.
_LABEL_BAD = re.compile(r"[\s\]]")


def _sub_level(depth: int) -> int:
    return min(MAX_HEADING_LEVEL, max(SUBHEAD_MIN_LEVEL, depth + 1))


def _finish_markdown(blocks: list) -> list[str]:  # type: ignore[type-arg]
    """Typed blocks → Markdown block strings, with the footnotes paired book-wide.

    The pairing rule is the one the .txt reader applies: a marker
    binds to the FIRST note line carrying the same label at or after it, in reading order. Books
    restart their numbering per chapter, so the same label names different notes in different
    chapters — and Markdown footnote labels are book-global. Every note therefore gets a label
    that is unique in the file: the book's own number when it is still free, else `number-N`.
    A marker with no note after it stays plain text (the reader shows it in bold, as for .txt);
    a note nobody cites stays a paragraph (a Markdown footnote nothing points at is not rendered,
    and the book's words must not vanish).
    """
    used: set[str] = set()
    labels: dict[int, str] = {}  # block index of a note -> its unique label
    resolved: dict[int, list[str | None]] = {}  # paragraph index -> label per marker, in order
    pending: dict[str, list[tuple[int, int]]] = {}  # core -> [(paragraph index, marker slot)]
    for i, block in enumerate(blocks):
        if block[0] == "p" or block[0] in ("h", "box"):
            text = block[-1]
            cores = NOTE_MARK_RE.findall(text)
            if cores:
                resolved[i] = [None] * len(cores)
                for slot, core in enumerate(cores):
                    pending.setdefault(core, []).append((i, slot))
        elif block[0] == "note":
            core = block[1]
            base = _LABEL_BAD.sub("", core) or "n"
            label, n = base, 1
            while label in used:
                n += 1
                label = f"{base}-{n}"
            refs = pending.pop(core, [])
            if refs:
                used.add(label)
                labels[i] = label
                for at, slot in refs:
                    resolved[at][slot] = label
    ...
    for i, block in enumerate(blocks):
        ...
        if kind == "blank":
            ...
            line = "#" * level + " " + _with_markers(md_escape(text), resolved.get(i))
            out.append(line + " {.box}" if kind == "box" else line)
        elif kind == "note":
            label = labels.get(i)
            if label is None:
                # nobody cites it: keep the words as an ordinary paragraph, label and all. The
                # label's brackets stay unescaped on purpose — after the indent they cannot start
                # a link reference definition, and the corpus comparison recognises the line by them.
                out.append(MD_INDENT + f"{md_escape(block[3])}[{block[1]}]: {md_escape(block[2])}")
            else:
                out.append(f"[^{label}]: " + md_escape(block[2]))
        else:
            out.append(MD_INDENT + _with_markers(md_escape(block[1]), resolved.get(i)))
    return out


def _with_markers(escaped: str, labels: "list[str | None] | None") -> str:
    """Turn each `(註[2])` marker into `[^label]`, positionally; unpaired ones stay as they are."""
    if not labels:
        return escaped
    slots = iter(labels)

    def swap(m: "re.Match[str]") -> str:
        label = next(slots, None)
        return f"[^{label}]" if label else m.group(0)

    return NOTE_MARK_RE.sub(swap, escaped)


def _join_blocks(blocks: list[str]) -> str:
    """Assemble the body: every block on its own, ONE blank line between blocks (that is what a
    paragraph break is in CommonMark), and every blank paragraph the source had adds one more
    blank line — the reader shows N blank lines as N−1, so one source blank shows as one line
    Blanks before the first block and after the last are dropped."""
    text = ""
    pending = 0
    for block in blocks:
        if block == "":
...
def _paragraph_blocks(
    paragraphs: list[str],
    blanks: list[int],
    indent: str,  # noqa: ARG001 - the indent is added by [_finish_markdown]
    heads: list[int] | None = None,
    after_heading: bool = False,
    depth: int = 1,
) -> list:  # type: ignore[type-arg]
    """Body paragraphs as typed blocks, with the source's own empty paragraphs between them.

    The spacing rules are the .txt format's (one blank line each side of a sub-heading, the
    book's own blank runs elsewhere — see EPUB2txt for the reasoning); only the SHAPE of each
    block changes: a sub-heading is a `###` heading (boxed: `### … {.box}`), the book's rule is
    `---`, a note line is a footnote definition, everything else a paragraph.
    """
    ...
    for position, paragraph in enumerate(paragraphs):
        ...
        note = NOTE_LINE_RE.match(paragraph) if kind == 0 else None
        if paragraph == HORIZONTAL_RULE:
            out.append(RULE_BLOCK)
        elif note:
            word = paragraph[:paragraph.index("[")]
            out.append(("note", note.group(1), note.group(2), word))
        elif kind == SUBHEAD_BOXED:
            out.append(("box", _sub_level(depth), paragraph))
        elif kind == SUBHEAD_PLAIN:
            out.append(("h", _sub_level(depth), paragraph))
        else:
            out.append(("p", paragraph))  # a caption keeps the indent, as in the .txt format
        if kind:
...
def _section(depth: int, label: str, paragraphs: list[str], indent: str = "",
             blanks: list[int] | None = None, heads: list[int] | None = None) -> list:  # type: ignore[type-arg]
    """`# Title`, one `#` per TOC level (capped at six, CommonMark's limit).

    The reader lists `#` and `##` in its table of contents; deeper TOC levels of the source
    become in-chapter headings — the words are kept, the TOC entry is not.
    """
    level = min(max(1, depth), MAX_HEADING_LEVEL)
    blocks: list = [("h", level, label)]  # type: ignore[type-arg]
    blocks.extend(_paragraph_blocks(paragraphs, blanks or [0] * len(paragraphs), indent, heads, depth=level))
    return blocks
...
def target_for(book: Path, root: Path | None, outdir: Path | None) -> Path:
    ...
    """
    if outdir is None:
        return book.with_suffix(".md")
    if root is None:
        return outdir / (book.stem + ".md")
    return outdir / book.relative_to(root).with_suffix(".md")
...
def convert_to(book: Path, dst: Path, with_cover: bool = True, cover_size: tuple[int, int] = COVER_MAX,
               cover_quality: int = COVER_QUALITY) -> tuple[str, int]:
    """Convert one book onto disk. Returns (title, characters written)."""
    title, text = convert(book, with_cover, cover_size, cover_quality)
    dst.parent.mkdir(parents=True, exist_ok=True)
    # Plain UTF-8, no BOM: Markdown tools expect none, and Reboku reads either.
...
# The two constants above, the five patterns below and the verdict thresholds (3 pages,
# 70%) are kept byte-for-byte identical with the Reboku reader app's import-time
# detector, so a book is judged the same way here and on the device. The app's test
# suite reads this file and fails if any of them drifts — change them in both places.
...
def run(jobs, policy: OverwritePolicy, report, stop=None, with_cover: bool = True,  # type: ignore[no-untyped-def]
        cover_size: tuple[int, int] = COVER_MAX, cover_quality: int = COVER_QUALITY) -> dict[str, int]:
    ...
    for book, dst in jobs:
        ...
        try:
            title, size = convert_to(book, dst, with_cover, cover_size, cover_quality)
...
def _run_cli(args) -> int:  # type: ignore[no-untyped-def]
    ...
    if not pillow_available() and not args.no_cover:
        print("Note: Pillow is not installed — books are converted without a cover "
              "(no cover: line in the header). `pip install pillow` to embed one.")
    # Comics and scans hold no text to extract, so they are reported once, up front,
    # rather than converted into an empty .md.
    ...
    tally = run(jobs, OverwritePolicy(_ask_console, args.force), _report_console,
                with_cover=not args.no_cover, cover_size=args.cover_size,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="EPUB2md",
        description="Convert EPUB books into Reboku Markdown books (.md).",
    ...
    parser.add_argument("-f", "--force", action="store_true",
                        help="overwrite existing .md without asking")
    parser.add_argument("--flat", action="store_true",
                        help="put every .md straight into -o, without rebuilding the "
                             "source sub-folder structure")
    parser.add_argument("--no-cover", action="store_true",
                        help="do not embed the cover picture (the header then has "
                             "no cover: line)")
    parser.add_argument("--cover-size", type=parse_cover_size, default=COVER_MAX, metavar="WxH",
                        help=f"box the cover is fitted inside, keeping its ratio "
                             f"(default {COVER_MAX[0]}x{COVER_MAX[1]})")
    parser.add_argument("--cover-quality", type=parse_cover_quality, default=COVER_QUALITY, metavar="N",
                        help=f"JPEG quality of the cover, {COVER_QUALITY_RANGE[0]}-{COVER_QUALITY_RANGE[1]} "
                             f"(default {COVER_QUALITY})")
    ...
    if args.self_test:
        ...
        try:
            ...
            print("It lives with the source: https://github.com/Dino9021/Reboku_EPUB2md")
            return
        selftest.run()  # prints "self-test OK" itself
        return
...
TEXT_ZH = {
    ...
    "lang_zh": "中文",
    "lang_en": "English",
    "title": "EPUB2md — Reboku Markdown 書轉換器",
    "source": "來源",
    "output": "輸出",
    "browse": "瀏覽",
    ...
    "cover_size": "封面尺寸",
    "cover_quality": "品質",
    "bad_cover": "封面尺寸或品質的格式不對,這次改用預設值 {size} / {quality}",
    "refresh": "重新整理",
    "found": "來源清單(勾選要轉換的書)",
    "queue": "轉換佇列",
    "add": "加入 →",
    ...
    "no_pillow": "沒裝 Pillow,轉出的書不會夾帶封面(表頭沒有 cover 欄);要封面請執行 pip install pillow",
}
...
TEXT_EN = {
    ...
    "lang_zh": "中文",
    "lang_en": "English",
    "title": "EPUB2md — Reboku Markdown book converter",
    "source": "From",
    "output": "To",
    "browse": "Browse",
    ...
    "bad_cover": "The cover size or quality is not valid; using the defaults {size} / {quality} this time",
    "refresh": "Refresh",
    ...
    "no_pillow": "Pillow is not installed, so books convert without their cover (no cover: line in the header). For covers: pip install pillow",
}
...
STATE_FILE = Path(os.environ.get("LOCALAPPDATA") or Path.home()) / "EPUB2md" / "state.json"
...
class _Gui:
    ...
    def __init__(self, root, source: Path | None, output: Path | None) -> None:  # type: ignore[no-untyped-def]
        ...
        self.cover_size_var = tk.StringVar(value=str(state.get("cover_size", f"{COVER_MAX[0]}x{COVER_MAX[1]}")))
        self.cover_quality_var = tk.StringVar(value=str(state.get("cover_quality", COVER_QUALITY)))
    ...
    def _build_paths(self) -> None:
        ...
        # Size and quality of that cover (owner 2026-09-23): small free-text fields, validated
        # when a run starts (see _start), so a half-typed value never blocks the window.
        ttk.Label(options, text=TEXT["cover_size"]).pack(side="left", padx=(14, 0))
        ttk.Entry(options, textvariable=self.cover_size_var, width=9).pack(side="left", padx=(4, 0))
        ttk.Label(options, text=TEXT["cover_quality"]).pack(side="left", padx=(10, 0))
        ttk.Entry(options, textvariable=self.cover_quality_var, width=4).pack(side="left", padx=(4, 0))
    ...
    def start(self) -> None:
        ...
        # The size / quality fields are free text: a value that does not parse falls back to
        # the defaults and says so in the log, rather than refusing the whole run.
        try:
            cover_size = parse_cover_size(self.cover_size_var.get())
            cover_quality = parse_cover_quality(self.cover_quality_var.get())
        except ValueError:
            cover_size, cover_quality = COVER_MAX, COVER_QUALITY
            self.write_log(TEXT["bad_cover"].format(size=f"{COVER_MAX[0]}x{COVER_MAX[1]}",
                                                    quality=COVER_QUALITY))

        def work() -> None:
            tally = run(jobs, policy, lambda *event: self.events.put(event),
                        stop=self.stop_flag.is_set, with_cover=with_cover,
                        cover_size=cover_size, cover_quality=cover_quality)
    ...
    def _remember(self) -> None:
        _save_state({
            ...
            "cover_size": self.cover_size_var.get().strip(),
            "cover_quality": self.cover_quality_var.get().strip(),
            "lang": self.lang,
        })
